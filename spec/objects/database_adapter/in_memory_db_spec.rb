...
describe Obj::DatabaseAdapter::InMemoryDb do
  ...
  describe '#unsaved?' do
    ...
    it 'returns true when an object is changed' do
      subject.add_obj(A.new(1,2))
      subject.write
      a_obj = subject.objs[:a].values.first
      a_obj.x = 10
      a_obj.save
      expect(subject.unsaved?).to be_truthy
    end
